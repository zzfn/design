...
files.forEach(async file =>{
    const componentInfo=parse(path.resolve(__dirname,`../../ui/components/${file}/${file}.tsx`))
    const has=await fs.stat(path.resolve(__dirname,`../src/${file}`))
    if(!has){
        await fs.mkdir(path.resolve(__dirname,`../src/${file}`))
    }
    await fs.writeFile(path.resolve(__dirname,`../src/${file}/types.md`), commentToMarkDown(componentInfo))
})


function commentToMarkDown(componentInfo) {
    return componentInfo.map(item=>{
        let { props }=item
